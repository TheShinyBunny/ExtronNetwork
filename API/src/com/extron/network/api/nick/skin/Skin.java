...
public class Skin {
    ...
    private static final Gson legacyGson = new GsonBuilder()
            .registerTypeAdapter(UUID.class, new UUIDTypeAdapter())
            .registerTypeAdapter(TextureModel.class, new TextureAdapter())
            .create();
    ...
    public Skin(int rowId, long timestamp, UUID uuid, String name
    ...
    public static Skin from(String encodedData, String signature) {
        byte[] data = Base64.getDecoder().decode(encodedData);
        String rawJson = new String(data, StandardCharsets.UTF_8);

        Skin skinModel = gson.fromJson(rawJson, Skin.class);
